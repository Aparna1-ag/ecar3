...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_changes'])) {
    ...
    // Update database
    $sql = "UPDATE suppliers SET fullname = ?, email_id = ?, contact_number = ?, address = ? WHERE supplier_id = ?";
...
if (isset($_POST['addWarehouse'])) {
    ...
    $email = $_POST['email'];
    $type_role = "suppliers";
    $temp_pass = md5("welcome");

    // Prepare SQL statement with placeholders
    $sql = "INSERT INTO suppliers (fullname, email_id, contact_number, address,type_role,username,password) VALUES (?, ?, ?, ?,?,?,?)";

    // Prepare and bind parameters
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssss", $warehouseName, $email, $phone, $address, $type_role, $email, $temp_pass);
